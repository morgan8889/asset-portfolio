...
function createPriceData(overrides: Partial<LivePriceData> = {}): LivePriceData {
  return {
    symbol: 'AAPL',
    price: '150.00',
    displayPrice: '150.00',
...
describe('useLivePriceMetrics', () => {
  ...
  describe('with no live prices', () => {
    it('returns hasLivePrices: false', () => {
      ...
      const { result } = renderHook(() => useLivePriceMetrics(holdings, assets));
  ...
  describe('with live prices', () => {
    ...
    it('calculates dayChangePercent correctly', () => {
      const holdings = [createHolding({ quantity: new Decimal(10) })];
      const assets = [createAsset({ symbol: 'AAPL' })];
      mockPrices.set('AAPL', createPriceData({
        displayPrice: '150.00',
        change: '2.50',
        changePercent: 1.69,
      }));
  ...
  describe('top performers', () => {
    it('returns holdings sorted by gain percent DESC', () => {
      const holdings = [
        createHolding({ id: 'h1', assetId: 'a1', quantity: new Decimal(10), costBasis: new Decimal(1000) }),
        createHolding({ id: 'h2', assetId: 'a2', quantity: new Decimal(10), costBasis: new Decimal(500) }),
        createHolding({ id: 'h3', assetId: 'a3', quantity: new Decimal(10), costBasis: new Decimal(2000) }),
      ];
      const assets = [
      ...
      mockPrices.set('AAPL', createPriceData({ symbol: 'AAPL', displayPrice: '150.00' }));  // 50% gain
      mockPrices.set('MSFT', createPriceData({ symbol: 'MSFT', displayPrice: '100.00' }));  // 100% gain
      mockPrices.set('GOOGL', createPriceData({ symbol: 'GOOGL', displayPrice: '180.00' })); // -10% loss
    ...
    it('filters out holdings with negative gains', () => {
      const holdings = [
        createHolding({ id: 'h1', assetId: 'a1', quantity: new Decimal(10), costBasis: new Decimal(2000) }),
      ];
    ...
    it('limits to top 5 performers', () => {
      ...
      holdings.forEach((_, i) => {
        mockPrices.set(`SYM${i}`, createPriceData({
          symbol: `SYM${i}`,
  ...
  describe('biggest losers', () => {
    it('returns holdings sorted by gain percent ASC (most negative first)', () => {
      const holdings = [
        createHolding({ id: 'h1', assetId: 'a1', quantity: new Decimal(10), costBasis: new Decimal(1500) }),
        createHolding({ id: 'h2', assetId: 'a2', quantity: new Decimal(10), costBasis: new Decimal(2000) }),
      ];
      const assets = [
        createAsset({ id: 'a1', symbol: 'AAPL' }),
        createAsset({ id: 'a2', symbol: 'MSFT' }),
      ];
      mockPrices.set('AAPL', createPriceData({ symbol: 'AAPL', displayPrice: '100.00' })); // -33% loss
      mockPrices.set('MSFT', createPriceData({ symbol: 'MSFT', displayPrice: '100.00' })); // -50% loss
    ...
    it('filters out holdings with positive gains', () => {
      const holdings = [
        createHolding({ id: 'h1', assetId: 'a1', quantity: new Decimal(10), costBasis: new Decimal(500) }),
      ];
      const assets = [createAsset({ id: 'a1', symbol: 'AAPL' })];
  ...
  describe('currency handling', () => {
    it('uses displayPrice (already converted from pence)', () => {
      ...
      mockPrices.set('VOD.L', createPriceData({
        symbol: 'VOD.L',
        price: '7250', // pence
