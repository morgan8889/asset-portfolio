...
describe('Validation Utilities', () => {
  describe('sanitizeInput', () => {
    ...
    it('should trim whitespace and limit length', () => {
      const input = '  Very long portfolio name that exceeds twenty characters  '
      const result = sanitizeInput(input)
  ...
  describe('validateApiResponse', () => {
    ...
    it('should throw error for invalid API response', () => {
      ...
      expect(() => validateApiResponse(schema, invalidData)).toThrow('Validation error')
    })
  })
  ...
  describe('validateExportRequest', () => {
    ...
    it('should reject invalid formats', () => {
      const data = [{ id: 1 }]
      expect(() => validateExportRequest('xml', data)).toThrow('Invalid export format')
    })

    it('should reject non-array data', () => {
      expect(() => validateExportRequest('json', 'not an array' as any)).toThrow('Data must be an array')
    })

    it('should reject too much data', () => {
      const largeData = Array.from({ length: 10001 }, (_, i) => ({ id: i }))
      expect(() => validateExportRequest('json', largeData)).toThrow('Too many records to export')
    })
